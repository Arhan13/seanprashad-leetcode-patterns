...
private:
    ...
    void dfs(int row, int col, set<pair<int, int>> &cells, int rows, int cols, int previousHeightOfCell, vector<vector<int>> &heights)
    {
        // Now check if we can do a DFS on this cell
        // Check the bounds
        // Check if the height[row][col] >= previousHeightOfCell
        // Check if the cell has already been visited
        if (!isInBounds(row, col, rows, cols) || previousHeightOfCell > heights[row][col] || cells.find({row, col}) != cells.end())
        {
            return;
        }
...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>> &heights)
    {
        ...
        set<pair<int, int>> pacCells;
        set<pair<int, int>> atlCells;
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < cols; col++)
            {
                ...
                if (pacCells.find(p) != pacCells.end() && atlCells.find(p) != atlCells.end())
                {
